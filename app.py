...
def save_charts_for_pdf(data, customer_metrics, y_test, test_preds, test_dates):
    charts = {}

    # Customer Segmentation Pie Chart
    fig1, _, _ = plot_segmentation_analysis(customer_metrics)
    chart_path1 = "customer_segment_pie_chart.png"
    pio.write_image(fig1, chart_path1, format="png", width=800, height=600)
    charts["Customer Segment Distribution"] = chart_path1

    # Sales Forecast Chart
    forecast_fig = plot_forecast_results(y_test, test_preds, test_dates)
    chart_path2 = "sales_forecast_chart.png"
    pio.write_image(forecast_fig, chart_path2, format="png", width=800, height=600)
    charts["Sales Forecast vs Actual"] = chart_path2

    return charts

def generate_pdf_report(overview, daily, hourly, stats, recommendations, selected_sections, charts=None):
    pdf = FPDF()
    pdf.add_page()
    ...
    sections_map = {
        "Overview": ("Key Performance Metrics", overview),
        "Daily Analysis": ("Daily Performance Metrics", daily),
        "Hourly Analysis": ("Hourly Performance Metrics", hourly),
        "Predictions": ("Statistical Insights", stats),
        "Recommendations": ("Strategic Recommendations", recommendations)
    }

    for section in selected_sections:
        if section in sections_map:
            title, data = sections_map[section]
    ...
    if uploaded_file:
        try:
            ...
            report_sections = st.sidebar.multiselect(
                ...
                ["Overview", "Daily Analysis", "Hourly Analysis", "Predictions", "Recommendations"],
                ...
                with st.spinner("Generating comprehensive PDF report..."):
                    ...
                    if include_charts:
                        ...
                        charts = {
                            ...
                            "Average Purchase by Segment": "average_purchase_by_segment.png",
                            "Total Revenue by Segment": "total_revenue_by_segment.png"
                        }

                        # Save charts as images
                        pio.write_image(fig1, charts["Customer Segment Distribution"], format="png", width=800, height=600)
                        pio.write_image(fig2, charts["Average Purchase by Segment"], format="png", width=800, height=600)
                        pio.write_image(fig3, charts["Total Revenue by Segment"], format="png", width=800, height=600)
                    ...
                    pdf_file = generate_pdf_report(
                        ...
                        stats,
                        recommendations,
                        selected_sections=report_sections,
                        charts=charts
                    )
